...
var ROI = ee.Geometry.Point([29.8385, -20.3130]).buffer(5000); 
var START_DATE = '2022-01-01';
var END_DATE = '2025-12-31';
var ACTIVE_GEOM = ROI; // Default to full mine monitoring area
 ...
 * 500-tree classifier trained on actual mine sample point locations.
 * Training labels derived from spatial context:
 *   0 = Compliant (dam centre, upstream clean water)
 *   1 = Caution (inlet, feeder channels, downstream)
 *   2 = Hazard (tailings adjacent, pipeline outfall)
 * Source: Mine Datasets/Sample Points.kml (34 GPS points)
 */
var TrainingPoints = ee.FeatureCollection([
  // Compliant — clean reservoir and upstream locations
  ee.Feature(ee.Geometry.Point([29.84650, -20.32087]), { 'status': 0, 'label': 'Gorge Dam Center' }),
  ee.Feature(ee.Geometry.Point([29.84738, -20.31960]), { 'status': 0, 'label': 'Gorge Dam NE' }),
  ee.Feature(ee.Geometry.Point([29.84798, -20.32020]), { 'status': 0, 'label': 'Gorge Dam Wall' }),
  ee.Feature(ee.Geometry.Point([29.82836, -20.31625]), { 'status': 0, 'label': 'Upstream River W' }),
  ee.Feature(ee.Geometry.Point([29.82943, -20.31409]), { 'status': 0, 'label': 'Upstream River NW' }),
  // Caution — transitional zones (inlets, feeders, downstream)
  ee.Feature(ee.Geometry.Point([29.84488, -20.32072]), { 'status': 1, 'label': 'Dam Inlet West' }),
  ee.Feature(ee.Geometry.Point([29.84459, -20.31964]), { 'status': 1, 'label': 'Inlet Channel' }),
  ee.Feature(ee.Geometry.Point([29.84361, -20.32071]), { 'status': 1, 'label': 'Upper Reservoir S' }),
  ee.Feature(ee.Geometry.Point([29.83330, -20.32195]), { 'status': 1, 'label': 'Downstream S' }),
  ee.Feature(ee.Geometry.Point([29.82989, -20.32009]), { 'status': 1, 'label': 'Downstream SW' }),
  // Hazard — tailings, pipeline, and industrial outfall
  ee.Feature(ee.Geometry.Point([29.84170, -20.31781]), { 'status': 2, 'label': 'Tailings E' }),
  ee.Feature(ee.Geometry.Point([29.84207, -20.31599]), { 'status': 2, 'label': 'Tailings Outfall' }),
  ee.Feature(ee.Geometry.Point([29.84032, -20.31497]), { 'status': 2, 'label': 'Pipeline N' }),
  ee.Feature(ee.Geometry.Point([29.83903, -20.31427]), { 'status': 2, 'label': 'Pipeline Intake' }),
]);
...
methodCard.add(ui.Label('This system utilizes Sentinel-2 MSI data with atmospheric correction. Machine Learning (Random Forest, 500 trees) is applied to classify water quality into 3 compliance tiers based on Mimosa Mine potable water standards (pH 6.5-7.5, TSS 0-1 mg/L, E.coli=0, Coliform<1000, Cl₂ 0.2-5 mg/L, EC<400 µS/cm).', { fontSize: '10px', color: '#6b7280' }));
...
// --- COMPLIANCE LEGEND (MIMOSA MINE STANDARDS) ---
...
legend.add(ui.Label('MIMOSA MINE COMPLIANCE STATUS', { fontSize: '11px', fontWeight: 'bold' }));
legend.add(makeLegendRow('#10b981', 'Compliant — TSS ≤1 mg/L, pH 6.5–7.5'));
legend.add(makeLegendRow('#f59e0b', 'Caution — Approaching limits'));
legend.add(makeLegendRow('#dc2626', 'Non-Compliant — Exceeds mine standards'));
legend.add(ui.Label(
  'pH 6.5-7.5 | TSS 0-1 | E.coli=0\nColiform <1000 | Cl₂ 0.2-5 | EC <400',
  { fontSize: '9px', color: '#6b7280', whiteSpace: 'pre', margin: '4px 0 0 0' }
));
